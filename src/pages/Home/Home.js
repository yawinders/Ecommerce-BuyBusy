...
import { addCartItemToFirebase, cartSelector } from '../../redux/reducers/cartReduer';
...
export default function Home() {
    ...
    const { items: products, status, error } = useSelector(productSelector)
    ...
    if (status === "loading") {
        return <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Loader /></div>;
    }

    if (status === "failed") {
        return <p style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>Error in fetching Products: {error}</p>;
    }
